...
async fn main(){
    ...
    // The directory where the React build is placed inside the container.
    let frontend_dir = std::path::PathBuf::from("./dist");
    
    // Fallback service to serve the index.html for all non-API routes (SPA routing)
    ...
    // Combine API routes and static file service
    let app = Router::new()
        .nest("/api", api_routes) // Prefix all API routes with /api
        .fallback_service(static_service); // Serve static files for everything else
